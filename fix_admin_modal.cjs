...
const search = `                          marginTop: '1rem',
                          margin: '1rem 0 0 0'
                position: 'relative',`;

const restore = `                          marginTop: '1rem',
                          margin: '1rem 0 0 0'
                        }}>
...
content = content.replace(search, restore);

// And append modal to the very end properly
if (!content.includes('품절 관리 모달')) {
  const modalHTML = `      {/* 품절 관리 모달 */}
      {stockModalProduct && (
...
`;
  content = content.replace('    </>\r\n  );\r\n}\r\n\r\nexport default Admin;', modalHTML + '}\r\n\r\nexport default Admin;');
  content = content.replace('    </>\n  );\n}\n\nexport default Admin;', modalHTML + '}\n\nexport default Admin;');
}

fs.writeFileSync(path, content);
console.log('Restored and inserted modal');
