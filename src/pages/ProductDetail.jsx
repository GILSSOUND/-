          ...
          <div ref={reviewRef} style={{ paddingTop: '2rem', paddingBottom: '5rem', borderTop: '1px solid #eee' }}>
            ...
            {(() => {
              ...
              return (
                <>
                  {/* 리뷰 통계 박스 */}
                  <div style={{ display: 'flex', flexWrap: 'wrap', background: '#f8f9fa', borderRadius: '12px', border: '1px solid #eee', marginBottom: '1rem', overflow: 'hidden' }}>
                    
                    {/* 왼쪽: 총평점 */}
                    <div style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: '#fff' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        <h4 style={{ fontSize: '1.1rem', margin: 0, color: '#333' }}>총 평점</h4>
                        <Star size={22} fill="#ffc107" color="#ffc107" />
                        <span style={{ fontSize: '1.8rem', fontWeight: 'bold', color: '#000', lineHeight: 1 }}>{avgRating}</span>
                      </div>
                      <p style={{ color: '#666', margin: '0.4rem 0 0 0', fontSize: '0.9rem' }}>구매후기 <strong style={{color: 'var(--primary-color)'}}>{totalReviews}</strong>건</p>
                    </div>
                    ...
                    {/* 오른쪽: 별점 분포 */}
                    <div style={{ flex: '2 1 300px', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '1rem', gap: '0.4rem', background: '#fafafa' }}>
                      {[
                        { stars: 5, label: '완전좋아요' },
                        { stars: 4, label: '좋아요' },
                        { stars: 3, label: '괜찮아요' },
                        ...
                        <div key={item.stars} style={{ display: 'flex', alignItems: 'center', fontSize: '0.9rem' }}>
                          <span style={{ width: '80px', color: '#444', fontWeight: 'bold' }}>{item.label}</span>
                  ...
                  {totalReviews === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                      {reviews.map(review => (
                        <div key={review._id} style={{ padding: '1rem', background: '#fff', borderRadius: '8px', border: '1px solid #eaeaea', boxShadow: '0 1px 4px rgba(0,0,0,0.02)' }}>
                          
                          {/* 작성자 및 별점 헤더 */}
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem', borderBottom: '1px dashed #eee', paddingBottom: '0.6rem' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                              <div style={{ display: 'flex', color: '#ffc107', gap: '0.1rem' }}>
                                {[1, 2, 3, 4, 5].map(star => (
                                  <Star key={star} size={14} fill={review.rating >= star ? '#ffc107' : 'none'} color={review.rating >= star ? '#ffc107' : '#ddd'} />
                                ))}
                              </div>
                              ...
                              {review.purchasedItems && review.purchasedItems.length > 0 && (
                                <div style={{ fontSize: '0.8rem', color: '#777', marginTop: '0.2rem' }}>
                                  {review.purchasedItems.map((item, i) => (
                                    <span key={i}>{item.name} {item.option ? `(${item.option})` : ''}{i < review.purchasedItems.length - 1 ? ', ' : ''}</span>
                                  ))}
                                </div>
                              )}
                            </div>
                          ...
                          {/* 2. 텍스트 영역 (나중에 표시) */}
                          <p style={{ lineHeight: '1.5', color: '#333', margin: 0, whiteSpace: 'pre-wrap', fontSize: '0.95rem', fontWeight: 400, fontFamily: '"Pretendard", "Noto Sans KR", sans-serif', wordBreak: 'keep-all' }}>
                            {review.content}
                          </p>
                        </div>
                      ))}
